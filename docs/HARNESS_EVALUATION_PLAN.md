...
### 2. Plan review rejection
plan review가 `not_approved`일 때 build 단계로 넘어가면 안 된다.
- 기대: `blocked` 또는 `planning_pending`
- 금지: `build_pending` 진입

### 3. Result review rejection
result review가 거절되면 validation으로 넘어가면 안 된다.
- 기대: `blocked` 또는 `build_pending`
- 금지: `validation_pending` 진입

### 4. Validation failure
validation이 실패하면 final_review로 넘어가면 안 된다.
- 기대: `build_pending` 또는 stop
- 금지: `final_review_pending` 진입
...
### 13. Reviewer evidence field pollution
...
### 16. field equality — ledger / trace / state 삼중 정렬
...
### 18. Acceptance missing at validation
`acceptance.md`가 없는 상태에서 `validation_pending`에 진입하면 차단되어야 한다.
- 기대: `blocked`, control-flow가 acceptance 누락을 보고
- 금지: validate-task 스크립트 실행, `final_review_pending` 진입

### 20. Bounded self-refine success
plan_review 또는 result_review에서 `approved_with_revisions`가 나왔을 때 PO가 bounded retry를 1회 수행하고 통과해야 한다.
- 기대: retry 후 review `approved`, 다음 단계 진행
- 금지: 2회 이상 자동 retry, scope 변경 허용

### 21. Bounded self-refine failure (2차 거절)
