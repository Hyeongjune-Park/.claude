...
- `plan_review`는 plan artifact만 본다.
- `result_review`는 build-summary artifact만 본다.
- `final_review`는 실제 파일과 기록된 validation output만 본다.
...
- `workflow_stage` (`plan_review`, `result_review`, `final_review` 중 하나)
- `artifact_type`
- `verdict`
- `next_allowed`
- `artifact_under_review`
